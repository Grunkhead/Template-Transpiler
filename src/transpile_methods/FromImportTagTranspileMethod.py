...
class FromImportTagTranspileMethod(TranspileMethodInterface):

    """ METHOD DESCRIPTION:
    
        Replace frontend path with relative backend theme path {% from <path> import ... %}
    """
    ...
    def execute(template, comparison_template):
        ...
        file_helper.write_file_content(
            absolute_file_path,
            r"(\{\%\s*from\s*\')([a-z-.\/]*" + comparison_template.name + ".html)(\'\s*import\s*[a-zA-Z]*\s*\%\})",
